...
import com.laptrinhweb.shoppo.entity.*;
import com.laptrinhweb.shoppo.service.*;
...
public class CartController {
	...
	String orderCheckout(User customer, Product product, HttpServletRequest request, HttpSession session,
	...
	@GetMapping("/my-cart")
	String showMyCartItems(User customer, HttpServletRequest request, HttpSession session, Model model) {
		try {
			session = request.getSession(false);
			String email = (String) session.getAttribute("email");
			if (email != null) {
	...
	String productCheckout(@RequestParam("code") String code, HttpServletRequest request, HttpSession session, 
			Product product, Model model, Cart cart, User customer) {
		try {
			session = request.getSession(false);
			String email = (String) session.getAttribute("email");
			if(email == null) {
	...
	String removeCartItems(@PathVariable("cid") Long cartId, HttpServletRequest request, HttpSession session,
			RedirectAttributes rda, User customer, Product product) {
		String email = "";
		try {
			session = request.getSession(false);
	...
	String cartItemsCheckout(@RequestParam("customerName") String customerName,
							 ...
							 Order order, OrderDetail od, Product product, User customer, RedirectAttributes rda) {
