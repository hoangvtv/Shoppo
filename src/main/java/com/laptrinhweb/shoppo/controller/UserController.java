...
public class UserController {
	...
	@GetMapping("/login")
	public String pageLogin(User customer, Model model) {
		model.addAttribute("customerLoginForm", new User());
		return "login";
	}

	@GetMapping("/register")
	public String pageRegister(User customer, Model model) {
		model.addAttribute("customerForm", new User());
		return "register";
	}

	@GetMapping("/forgot-password")
	public ModelAndView CustomerForgotPasswordPage(User Customer) {
	...
	public ModelAndView CustomerChangePasswordPage(@RequestParam("email") String encodedEmail,
		...
		User emailExist = customerService.findCustomerByEmail(email);
		if (emailExist == null) {
			return new ModelAndView("redirect:/page404");
		} else {
	...
	@GetMapping("/logout")
	public String CustomerLogoutPage(User customer, HttpSession session, HttpServletRequest request,
	...
	String createCustomer(@Valid @ModelAttribute("customerForm") User customer, BindingResult br, Model model,
			HttpSession session) {
		try {
			if (br.hasErrors()) {
...
//				customer.setRoles(userRole);
	...
	String validateCustomer(@Valid @ModelAttribute("customerLoginForm") User customer, BindingResult br,
	...
	public ModelAndView forgotPassword(@Valid @ModelAttribute("forgot-password") User Customer,
		...
		User CustomerExist = customerService.findCustomerByEmail(email);
	...
	public ModelAndView editCustomer(HttpServletRequest request, HttpSession session) {
		...
		User customer = customerService.findCustomerByEmail(email);
		if (customer != null) {
	...
	String updateCustomer(@Valid @ModelAttribute("customerUpdate") User customer, BindingResult br,
